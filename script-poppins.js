...
resumeDownloadPdf.addEventListener("click", async function() {
    ...
    // Load Poppins TTFs from Google Fonts repo (via jsDelivr) and base64-encode
    ...
    var poppinsFonts;
    try {
        var fontBase = "https://cdn.jsdelivr.net/gh/google/fonts@main/ofl/poppins/";
        poppinsFonts = await Promise.all([
            loadFontBase64(fontBase + "Poppins-Regular.ttf"),
            loadFontBase64(fontBase + "Poppins-Bold.ttf"),
            loadFontBase64(fontBase + "Poppins-Italic.ttf"),
            loadFontBase64(fontBase + "Poppins-BoldItalic.ttf")
        ]);
    } catch (err) {
        alert("Could not load Poppins font. Please check your internet connection.");
        return;
    }

    var pdf = new JsPDF("p", "mm", "a4");

    // Register Poppins with jsPDF
    pdf.addFileToVFS("Poppins-Regular.ttf", poppinsFonts[0]);
    pdf.addFont("Poppins-Regular.ttf", "Poppins", "normal");
    pdf.addFileToVFS("Poppins-Bold.ttf", poppinsFonts[1]);
    pdf.addFont("Poppins-Bold.ttf", "Poppins", "bold");
    pdf.addFileToVFS("Poppins-Italic.ttf", poppinsFonts[2]);
    pdf.addFont("Poppins-Italic.ttf", "Poppins", "italic");
    pdf.addFileToVFS("Poppins-BoldItalic.ttf", poppinsFonts[3]);
    pdf.addFont("Poppins-BoldItalic.ttf", "Poppins", "bolditalic");
    var pageW = 210;
    var pageH = 297;
    var m = 8;
    var cW = pageW - m * 2;
    ...
    for (var s = 0; s < skills.length; s++) {
        ...
        for (var vl = 0; vl < valLines.length; vl++) {
            ...
            if (vl < valLines.length - 1) y += skillLh;
        }
        y += 4.6;
    }
    y -= 4.6;
